...
# NOTE: Set Collision -> Safe Margin to something bigger than the default 0.001, e.g., 0.03
...
#@onready var gun:Node3D = $around/head/cam/gun
...
func teleport_to(dest : Node3D, _play_sound : bool = false) -> void:
...
func move_air(delta : float) -> void:
	...
	move_and_slide()
	_coltest()
	#_process_stair(delta)  # commented out because we don't want Q2 stair climbing while in the air
...
func _process_stair(delta : float) -> void:
	...
	if PhysicsServer3D.body_test_motion(
		...
		if (
			...
			tbodyr.get_collision_normal().angle_to(Vector3.UP) <= stairstep_deg
		) :
