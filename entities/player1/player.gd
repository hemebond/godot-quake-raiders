...
func _process(delta: float) -> void:
	#liblast_movement(delta)
	old_process(delta)


func old_process(delta: float) -> void:
...
@onready var _collision_debug_mesh = Player1.make_debug_mesh(Color(1.0, 0.75, 0.5, 0.5))
@onready var _collision_debug_mesh_unwalkable = Player1.make_debug_mesh(Color(1.0, 0.0, 0.0, 0.5))
...
#
# Liblast Controller Code
#

enum Medium {GROUND, AIR, WATER}
...
@export var joypad_look_sensitivity := 1.0
@export var joypad_deadzone := 0.1
@export var liblast_step_height := 0.4
...
@export var medium_accel: Dictionary[Medium, float] = {
	Medium.GROUND : 8.0,
	Medium.AIR : 1.0,
	Medium.WATER : 3.0,
}
...
func liblast_movement(delta: float) -> void:
	# if the mouse is not in captured mode, the character should not react to any inputs (like key strokes)
	input_active = Input.mouse_mode == Input.MOUSE_MODE_CAPTURED

	if not is_on_floor():
		velocity += get_gravity() * gravity_factor * delta
		medium = Medium.AIR

	elif is_on_floor() and medium == Medium.AIR:
	...
	if input_active:
		...
		current_speed = movement_speed * (sprint_factor if Input.is_action_pressed("move_sprint") else 1.0)
	...
	if not jump_active:
		pass#apply_floor_snap()

	if direction.length() > Vector2(get_real_velocity().x, get_real_velocity().z).length():
		#print(Engine.get_physics_frames())
		#print("Move blocked")
		if not test_move(global_transform.translated(Vector3.UP * liblast_step_height), velocity * delta):
			if not test_move(global_transform.translated(Vector3.UP * liblast_step_height), velocity.normalized() * step_depth):
				#print("STEP HEIGHT REACHABLE")
				var step_collision: KinematicCollision3D
				if test_move(global_transform.translated(Vector3.UP * liblast_step_height + velocity.normalized() * step_depth), Vector3.DOWN, step_collision):
					#print("STEPPING UP: ", inst_to_dict(step_collision))
					move_and_collide(Vector3.UP * liblast_step_height)
					move_and_collide(velocity * delta)
					move_and_collide(Vector3.DOWN * liblast_step_height)
					#move_and_collide(- velocity * delta) # move back so move_and_slide can do it's job
			else:
				pass
				#print("STEP TOO HIGH or TOO SHALLOW")
	move_and_slide()
	...
	#look(look.x * joypad_look_sensitivity, look.y * joypad_look_sensitivity)
	
#func look(x: float, y: float):
	#head.rotation.x = clamp(head.rotation.x + y * mouse_sensitivity, deg_to_rad(-89), deg_to_rad(89))
	#rotation.y += x * mouse_sensitivity
